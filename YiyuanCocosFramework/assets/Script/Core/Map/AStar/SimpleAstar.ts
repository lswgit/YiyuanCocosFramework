...
export default class SimpleAstar {
    ...
    public find(xnow: number, ynow: number, xpos: number, ypos: number): PathNode[] {
        xpos = Math.floor(xpos / MapGameData.GameCellWidth);
        ypos = Math.floor(ypos / MapGameData.GameCellHeight);
        xpos = Math.min(xpos, this._grid.numCols - 1);
        ypos = Math.min(ypos, this._grid.numRows - 1);
        this._grid.setEndNode(xpos, ypos); //1

        xnow = Math.floor(xnow / MapGameData.GameCellWidth);
        ynow = Math.floor(ynow / MapGameData.GameCellHeight);
...
export class AStar {
    ...
    private floydCrossAble(n1: PathNode, n2: PathNode): boolean {
        var ps: cc.Vec2[] = this.bresenhamNodes(new cc.Vec2(n1.x, n1.y), new cc.Vec2(n2.x, n2.y));
    ...
    private bresenhamNodes(p1: cc.Vec2, p2: cc.Vec2): cc.Vec2[] {
        ...
        if (steep) {
            ret.push(new cc.Vec2(p1.y, p1.x));
        } else {
            ret.push(new cc.Vec2(p1.x, p1.y));
        }
        while (nowX != p2.x) {
            var fy: number = Math.floor(nowY)
            var cy: number = Math.ceil(nowY);
            if (steep) {
                ret.push(new cc.Vec2(fy, nowX));
            } else {
                ret.push(new cc.Vec2(nowX, fy));
            }
            if (fy != cy) {
                if (steep) {
                    ret.push(new cc.Vec2(cy, nowX));
                } else {
                    ret.push(new cc.Vec2(nowX, cy));
                }
            }
            nowX += stepX;
            nowY += deltay;
        ...
        if (steep) {
            ret.push(new cc.Vec2(p2.y, p2.x));
        } else {
            ret.push(new cc.Vec2(p2.x, p2.y));
        }
        return ret;
    }
